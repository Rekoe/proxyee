...
public class HttpProxyInitializer extends ChannelInitializer<Channel> {
	...
	protected void initChannel(Channel ch) throws Exception {
		...
		if (requestProto.getSsl()) {
			ch.pipeline().addLast(((HttpProxyServerHandle) clientChannel.pipeline().get("serverHandle")).getServerConfig().getClientSslCtx().newHandler(ch.alloc(), requestProto.getHost(), requestProto.getPort()));
		}
